	...
	function big_scroll(a,t_jian,step,direction,lr_btn,pre_num) {
		...
		if(!pagenation_html){
			...
			$('#'+a).delegate(".pagenation span","touchend",function(){
			...
			$('#'+a).delegate(".pagenation_scroll_inner .inner","touchend",function(){
			    var index=$(this).index()+1;
			    clearInterval(time_jian);
				clearInterval(time_move);
				clearInterval(time_pagenation);
			...
			$('#'+a+' .pagenation_scroll').on('touchstart',function(e) {
				event.stopPropagation();
				event.preventDefault();
				if(direction_chuan=='vertical'){
					var pagenation_top_now=$('#'+a+' .pagenation_scroll').scrollTop();
					var linjie=$('#'+a+' .pagenation_scroll_inner').height()-$('#'+a+' .pagenation_scroll').height()
			    	var top_now_start= e.originalEvent.targetTouches[0].pageY+pagenation_top_now;
					$('#'+a+' .pagenation_scroll').on('touchmove',function(e) {
					  	var top_now_end=  e.originalEvent.targetTouches[0].pageY;
					  	pagenation_top_now=top_now_start-top_now_end;
						if(pagenation_top_now>=linjie){
							pagenation_top_now=linjie;
						}else if(pagenation_top_now<=0){
							pagenation_top_now=0;
						}
						$('#'+a+' .pagenation_scroll').scrollTop(pagenation_top_now);
					});
				}else{
			    	var pagenation_left_now=$('#'+a+' .pagenation_scroll').scrollLeft();
					var linjie=$('#'+a+' .pagenation_scroll_inner').width()-$('#'+a+' .pagenation_scroll').width()
			    	var left_now_start= e.originalEvent.targetTouches[0].pageX+pagenation_left_now;
					$('#'+a+' .pagenation_scroll').on('touchmove',function(e) {
					  	var left_now_end=  e.originalEvent.targetTouches[0].pageX;
					  	pagenation_left_now=left_now_start-left_now_end;
						if(pagenation_left_now>=linjie){
							pagenation_left_now=linjie;
						}else if(pagenation_left_now<=0){
							pagenation_left_now=0;
						}
						$('#'+a+' .pagenation_scroll').scrollLeft(pagenation_left_now);
					});
				}
			});
			$('#'+a+' .pagenation_scroll').on('touchend',function(e) {
				$('#'+a+' .pagenation_scroll').on('touchmove',function(e) {
				  	return null;
				});
			});
